# Cybersecurity Job Simulations through Forage

<img src = "https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/Forage.png" width ="400" >

A collection of enterprise virtual job simulations I've completed through Forage, covering security operations, risk assessment, threat analysis, and incident response across four organizations.
 
Each simulation below links to a dedicated page with the program overview, the tasks I completed, and the skills demonstrated.

| Company | Focus Area | Format |
|---|---|---|
| [AIG](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/AIG.md) |  Log4Shell vulnerability response & ransomware decryption | CISA advisory + Python bruteforce script |
| [Mastercard](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/Mastercard.md) | Phishing simulation & security awareness | Security Analyst virtual experience |
| [Deloitte Australia](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/Deloitte.md) | Breach investigation via web log analysis | Cyber consultant — Daikibo Industrials case |
| [Datacom](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/Datacom.md) | Ransomware impact analysis & enterprise risk assessment | Analyst Graduate — Orion Health & RetailNova cases |
| [TCS](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/TCS.md) | IAM strategy, solution design & rollout planning | IAM Developer — TechCorp Enterprises engagement |
| [Commonwealth Bank](https://github.com/niha-v/Cybersecurity-Job-Simulations/blob/main/Commbank.md) | Fraud analytics, incident response, security awareness & pentesting | Cybersecurity Generalist — 4-part simulation | 

---

### About these simulations

These are self-paced, task-based programs designed by each company to mirror real day-to-day work on their security teams — not graded coursework, but hands-on scenarios with deliverables I produced myself (advisory emails, scripts, reports, and presentations).

📫 **Connect with me:** [LinkedIn](https://www.linkedin.com/in/niharikaumrani) · [Email](mailto:niharika.umrani@gmail.com)
